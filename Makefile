PIPELINE_FAMILY := invoices
PIPELINE_PACKAGE := invoices
...
.PHONY: docker-start-api
docker-start-api:
	docker run  -p 8000:8000 --mount type=bind,source=$(realpath .),target=/home/notebook-user/local -t --rm pipeline-family-invoices-dev:latest uvicorn prepline_invoices.api.invoices:app --host 0.0.0.0 --port 8000

.PHONY: docker-start-jupyter
docker-start-jupyter:
	docker run  -p 8888:8888 --mount type=bind,source=$(realpath .),target=/home/notebook-user/local -t --rm pipeline-family-invoices-dev:latest jupyter-notebook --port 8888 --ip 0.0.0.0 --no-browser --NotebookApp.token='' --NotebookApp.password=''
...
## test-sample-docs:            runs the pipeline on a set of sample invoice documents
